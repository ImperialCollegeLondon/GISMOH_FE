    ...
    Linker.Graph = Backbone.View.extend({
        ...
        createLegend : function()
        {
            ...
            legend_group.appendChild(this.createSVGElement('text', {
                ...
                .appendChild(document.createTextNode('Similar strain'));

            legend_group.appendChild(this.createSVGElement('path', {
                d : this.getOverlapShape(0, 25, this.d * 1.1),
                class : 'danger'
            }, true));

            var lbl2 = this.createSVGElement('text', {x:10 + this.d, y:30, r:this.d}, true),
                sp1 = this.createSVGElement('tspan', { x : 10 + this.d, dy: "-0.5em" }, true),
                sp2 = this.createSVGElement('tspan', { x : 10 + this.d, dy : "1em" }, true);

            sp1.appendChild(document.createTextNode('Similar strain'));
            sp2.appendChild(document.createTextNode('and location'));
        ...
        getOverlapShape : function(x, y, r) {
            var x_ = x - r,
                x$ = x + r,
                y_ = y - r,
                y$ = y + r;
        ...
        addOne : function (model, i) {
            ...
            var cen = this.getCentre(i, model.get('similarity')),
                l = this.createSVGElement('path', {d : 'M ' + this.centre[0] + ' ' + this.centre[1] + ' l' + cen[0] + ' ' + cen[1], id : this.d }),
                 c = this.createSVGElement('circle', {
                    ...
                    'data-isolate' : model.get('Isolate').isolate_id
                });
            ...
            var ct = this.getTextAnchor(i, model.get('similarity')),
                ...
                l1 = this.createSVGElement('tspan', { x : this.centre[0] + ct[0], dy : '-0.5em', 'text-anchor' : 'start' }),
                l2 = this.createSVGElement('tspan', { x : this.centre[0] + ct[0], dy : '1em' , 'text-anchor' : 'start'}),
                date_string = new Date(model.get('Isolate')['date_taken']).strftime('%d %b %Y');

            l1.appendChild(document.createTextNode(model.get('Isolate')['patient_number']));
            l2.appendChild(document.createTextNode(date_string + ' - ' + model.get('Isolate')['location'].replace(/\s-\s.*$/i, '')));
            lbl.appendChild(l1);
            lbl.appendChild(l2);
        },
        ...
        addOneLocation : function(model, i) {
            ...
            if(ele && ele.tagName == 'circle')
            {
                ...
                d : this.getOverlapShape(ele.cx.baseVal.value,
                                         ele.cy.baseVal.value,
                                         ele.r.baseVal.value * 1.1),
                    class : 'danger',
                    id: ele.id,
                    title : ele.title,
                    'data-isolate' : ele['data-isolate']
                });
        ...
        selectPatient : function(evt) {
            ...
            this.router.set_path({ patient : this.selected_id, isolate: "" });
        },
